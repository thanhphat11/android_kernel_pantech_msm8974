...
// p15060
	...
	if ((!strncmp(desc->name, "modem", 5) || !strncmp(desc->name, "wcnss", 5) ||
	     !strncmp(desc->name, "riva", 4))) {
...
void ssr_subsys_coupled_action(const char *name)
{
	...
	if (!strncmp(name, "lpass", 5)) {
		...
		snprintf(ssr_reason, sizeof(ssr_reason), "0x%XR", SYS_RESET_REASON_MODEM);   // post fix : "R"
#else
#if defined(CONFIG_MACH_MSM8974_EF56S) || defined(CONFIG_MACH_MSM8974_EF59S) || \
    ...
    defined(CONFIG_MACH_MSM8974_EF62L) || defined(CONFIG_MACH_MSM8974_EF65S)
		snprintf(ssr_reason, sizeof(ssr_reason), "0x%XR", SYS_RESET_REASON_MODEM);   // post fix : "R"
#else
		snprintf(ssr_reason, sizeof(ssr_reason), "0x%X", SYS_RESET_REASON_MODEM);
#endif
	...
	if ((!strncmp(name, "modem", 5) || !strncmp(name, "wcnss", 5) || !strncmp(name, "riva", 4)) &&
