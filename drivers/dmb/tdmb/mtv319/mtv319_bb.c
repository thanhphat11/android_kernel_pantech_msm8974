...
uint8 mtv319_init(void)
{
  ...
  TDMB_MSG_RTV_BB("[%s] res[%d]  [SDK Ver : 140605] \n", __func__, res); // EF65 : PLL unlock fix - 4 time lock check
  //TDMB_MSG_RTV_BB("[%s] res[%d]  [SDK Ver : 140603] \n", __func__, res); // EF65 : DAB play bug fix RTV_DAB_MSC_HDR_ENABLED
  //TDMB_MSG_RTV_BB("[%s] res[%d]  [SDK Ver : 140430] \n", __func__, res); //EF65 : RTV_PLAY_FIC_HDR_ENABLED, RTV_DAB_MSC_HDR_ENABLED
  //DMB_MSG_RTV_BB("[%s] res[%d]  [SDK Ver : 131129] \n", __func__, res); //EF60 : final SDK
  return res;
}
...
#else //After EF65, 0~4 5 step
  uint16 ant_tbl[] = {900,//900, // 0 <-> 1
                               700, // 1 <-> 2
                               500, // 2 <-> 3
                               300}; // 3 <-> 4

  uint16 hystery_value[]= {0,50,50,20};

  //TDMB_MSG_RTV_BB("[%s]\n", __func__);

  if(pcber < ant_tbl[3]) level = 4;//(ber >= 1 && ber <= 100) printf("ANT: 5\n");
  else if(pcber >= ant_tbl[3] && pcber < ant_tbl[2]) level = 3;//(ber >= 101 && ber <= 150) printf("ANT: 4\n");
  else if(pcber >= ant_tbl[2] && pcber < ant_tbl[1]) level = 2;//(ber >= 151 && ber <= 300) printf("ANT: 3\n");
  else if(pcber >= ant_tbl[1] && pcber < ant_tbl[0]) level = 1;//(ber >= 301 && ber <= 500) printf("ANT: 2\n");
  else if(pcber >= ant_tbl[0]) level = 0;//(ber >= 801) printf("ANT: 0\n");
#endif
