...
#define TDMB_REMOVE_NOISE_VAL 990
#define TDMB_RESTORE_VAL 980
#else
#define TDMB_REMOVE_NOISE_VAL 910
#define TDMB_RESTORE_VAL 900
#endif
#endif
...
U32 rtvTDMB_GetCER(void)
{
	...
	#ifdef RTV_REMOVE_NOISE_MSC
		if (cer > TDMB_REMOVE_NOISE_VAL && g_fRtvCerMscSet) {
			if (++msc_set_cnt == 3) {
				g_fRtvCerMscSet = FALSE;
				msc_set_cnt = 0;
			}
		} else if(cer < TDMB_RESTORE_VAL) {
...
#endif /* RTV_MULTIPLE_CHANNEL_MODE */
...
S32 rtvTDMB_GetRSSI(void)
{
	...
	U8 CH_FLAG,ADJRSSID,ADJRSSID_N;
#endif
	S32 nRssi = 0;
...
#ifdef DEBUG_LOG_FOR_RSSI_FITTING
	ADJRSSID = RTV_REG_GET(0x05);
	ADJRSSID_N = RTV_REG_GET(0x06);
#endif
...
#ifdef DEBUG_LOG_FOR_RSSI_FITTING
	RTV_DBGMSG3("[rtvTDMB_GetRSSI] Channel Flag = %d 0x00=0x%02x, 0x01=0x%02x 0x05=0x%02x  0x06=0x%02x\n",CH_FLAG, RD00, GVBB,ADJRSSID,ADJRSSID_N);
	RTV_DBGMSG3("LNAGAIN = %d, RFAGC = %d GVBB : %d\n", LNAGAIN, RFAGC,GVBB);
	RTV_DBGMSG3("[rtvTDMB_GetRSSI] 0x02(0x%02X), 0x0B(0x%02X)",RTV_REG_GET(0x02), RTV_REG_GET(0x0B));
#endif
...
|| defined(RTV_DMB_MSC_HDR_ENABLED) || defined(RTV_DAB_MSC_HDR_ENABLED)
...
#ifdef RTV_MULTIPLE_CHANNEL_MODE
    RTV_DBGMSG0("[rtvTDMB_Initialize] MULTIPLE CHANNEL enabled\n");
#else
