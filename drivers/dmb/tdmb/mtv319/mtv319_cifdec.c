...
*
* TITLE		: MTV319 CIF decoder source file for T-DMB and DAB.
*
* FILENAME	: mtv319_cifdec.c
*
* DESCRIPTION	:
...
*===========================================================================*/
#define MAX_NUM_INPUT_TSP	(RTV_SPI_CIF_MODE_INTERRUPT_SIZE/188)
	...
	decode_header(struct CIFB_INFO *cifb, const U8 *tsp)
{
	...
	if (tsp[0] == RTV_MCH_HEADER_SYNC_BYTE) { /* sync byte ok? */
	...
	#if !defined(RTV_DMB_MSC_HDR_ENABLED)
		if (g_eSingleSvcFlagID == RTV_MCH_HDR_ID_DMB) {
			if (tsp[0] == 0x47) {
				...
				CIF_DBGMSG1("[decode_headers] ERR tsp[0x%02X]\n", tsp[0]);
			}
		} else
			return CIFDEC_S__INVALID_SVC_TYPE;
	#endif
...
#ifdef RTV_REMOVE_NOISE_MSC
static unsigned long prev_ic_jiffies;
#endif
			...
			const U8 *pbTsBuf, UINT nTsLen)
{
	...
	#else
		for (i = 0; i < RTV_MAX_NUM_USE_SUBCHANNEL; i++)
			cifb_queue_release_all(&cifdec_msc_tspq[i]);
	#endif
